...
class AsyncTask:
    def __init__(self, args):
        ...
        self.metadata_scheme = MetadataScheme(
            args.pop()) if not args_manager.args.disable_metadata else MetadataScheme.PROTO
...
def worker():
    ...
    from modules.sdxl_styles import apply_style, get_random_style, proto_expansion, apply_arrays, random_style_name
    ...
    def save_and_log(async_task, height, imgs, task, use_expansion, width, loras) -> list:
        ...
        for x in imgs:
            d = [('Prompt', 'prompt', task['log_positive_prompt']),
                 ...
                 ('Styles', 'styles',
                  str(task['styles'] if not use_expansion else [proto_expansion] + task['styles'])),
            ...
            d.append(('Version', 'version', 'Proto v' + proto_version.version))
    ...
    def process_prompt(async_task, prompt, negative_prompt, base_model_additional_loras, image_number, disable_seed_increment, use_expansion, use_style,
        ...
        if use_expansion:
            ...
            for i, t in enumerate(tasks):
                ...
                progressbar(async_task, current_progress, f'Preparing Proto text #{i + 1} ...')
    ...
    def handler(async_task: AsyncTask):
        ...
        if proto_expansion in async_task.style_selections:
            use_expansion = True
            async_task.style_selections.remove(proto_expansion)
        else:
            use_expansion = False
